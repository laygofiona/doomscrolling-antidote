...
def get_papers(dailyrun_id):
    ...
    try:
        ...
        logging.error(f"Exception on get_papers(): Failed to connect to SQL database file: {e}")
        return []
    ...
    try:
        cursor.execute(
            "SELECT papers_ids FROM dailyRun WHERE id = ?",
            (dailyrun_id,)
        )
        row = cursor.fetchone()
        ...
        logging.error(f"Exception on get_papers(): Extracting papers_IDS from dailyRun {e}")
...
def get_id(type: str, dailyRun_id):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            f"SELECT {column} FROM dailyRun WHERE id = ?",
            (dailyRun_id,)
        )
        row = cursor.fetchone()
        if row:
        ...
        logging.error(f"Exception on get_id(): Failed to get {column} for dailyRun {dailyRun_id}: {e}")
    finally:
        if conn is not None:
            conn.close()
